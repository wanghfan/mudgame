...
void show_medal(people &player,int day)
{
    ...
    if(day>40)
        cout<<"********成功立足*********"<<endl;
    //IQ
    if (player.get_IQ()>=150)
        cout<<"*******绝顶聪明********"<<endl;
    if (player.get_IQ()<150&&player.get_IQ()>=100)
        cout<<"*******聪明伶俐********"<<endl;
    if (player.get_IQ()<100&&player.get_IQ()>=50)
        cout<<"*******知识分子********"<<endl;
    if (player.get_IQ()<50&&player.get_IQ()>=20)
        cout<<"*******呆头呆脑********"<<endl;
    if (player.get_IQ()<20)
        cout<<"*******智力障碍********"<<endl;
    //EQ
    if (player.get_EQ()>=150)
        cout<<"*******八面玲珑********"<<endl;
    if (player.get_EQ()<150&&player.get_IQ()>=100)
        cout<<"*******社交达人********"<<endl;
    if (player.get_EQ()<100&&player.get_IQ()>=30)
        cout<<"*******情商中等********"<<endl;
    if (player.get_EQ()<30){
        string male("m");
        string female("f");
        if(player.get_sex()==male){
            cout<<"*******钢铁直男********"<<endl;
        }
        else if(player.get_sex()==female){
            cout<<"*******钢铁直女********"<<endl;
        }
        else{
            cout<<"*******钢铁直无性人********"<<endl;
    ...
    //physique
    if (player.get_physique()>=150)
        cout<<"*******魔鬼筋肉人********"<<endl;
    if (player.get_physique()<150&&player.get_IQ()>=100)
        cout<<"*******大力士********"<<endl;
    if (player.get_physique()<100&&player.get_IQ()>=80)
        cout<<"*******肌肉发达********"<<endl;
    if (player.get_physique()<80&&player.get_IQ()>=30)
        cout<<"*******体测合格********"<<endl;
    if (player.get_physique()<30&&player.get_IQ()>=10)
        cout<<"*******弱不禁风********"<<endl;
    if (player.get_physique()<10)
        cout<<"*******奄奄一息********"<<endl;
    //money
    if (player.get_money() >= 200)
        cout << "*******富可敌国********" << endl;
    if (player.get_money() < 200 && player.get_money() >= 100)
        cout << "*******打工皇帝********" << endl;
    if (player.get_money() < 100 && player.get_money() >= 50)
        cout << "*******全面小康********" << endl;
    if (player.get_money() < 50 && player.get_money() >= 20)
        cout << "*******温饱线********" << endl;
    if (player.get_money() < 20 && player.get_money() >= 1)
        cout << "*******扶贫对象********" << endl;
...
void start_view() //起始的展示
{
    ...
    // cout << "按下任意键开始" << endl;
    // Sleep(delay_time);
    cout << "你出生在了一个陌生的城市，您将在这个城市生活一定天数，您可以自由生活，尽情探索这个城市。" << endl;
    Sleep(delay_time);
    system("pause");
    Sleep(delay_time);
    cout << "根据您的最终点数，我们会生成荣誉勋章" << endl;
    Sleep(delay_time);
    cout<<"注意，本游戏暂不支持中文输入"<<endl;
    cout << "请享受游戏!" << endl;
    Sleep(delay_time);
    cout << "按下任意键开始" << endl;
    Sleep(delay_time);
...
{
    Sleep(delay_time);
    cout<<"您现在一共有200点，请您分配到幸福、IQ、EQ、体魄、颜值上"<<endl;
    Sleep(delay_time);
